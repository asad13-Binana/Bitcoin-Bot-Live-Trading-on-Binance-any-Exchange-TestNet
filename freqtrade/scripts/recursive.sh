#!/usr/bin/env bash
# Offline-only recursive-indicator analysis for the exact configured BTC/stable pair.
...
OUTPUT_LOG="$OUTPUT_DIR/recursive-analysis-${PAIR_TAG}-${RUN_ID}.log"
...
state = {
    ...
    "generation": 1,
    "source": "offline-recursive-analysis",
...
# Freqtrade forces cache=none for this diagnostic. The selected odd startup
# counts mirror the official guidance around common exchange candle limits.
"${COMPOSE[@]}" run --rm --no-deps --cap-drop ALL \
  ...
  --timerange "$TIMERANGE" \
  --pairs "$PAIR" \
  --startup-candle 199 499 999 1999 2>&1 | tee "$OUTPUT_LOG"

echo "Recursive-analysis artifact retained at: $OUTPUT_LOG"
