...
| GitHub repository and Actions | PARTIAL / COMMIT-SPECIFIC | The user-owned private testnet and live repositories now exist and this source is a Git checkout. Each exact release commit must still pass the complete matrix and artifact job. Branch protection, any plan-dependent reviewers, the restricted `oracle-sim` runner and the one-use root-approved digest flow remain separate controls that must be configured or proven; they must not be inferred from repository creation. |
