...
## Shared-account and recovery follow-up

Fresh AWS evidence found a Bitcoin pause caused by unrelated LINKUSDT and UNIUSDT filled-stop events from the same Binance account. Unknown event symbols are now classified using authoritative exchange metadata before entering Bitcoin trade/risk processing. Verified non-BTC events do not pause Bitcoin or schedule partial-entry recovery. Missing, mismatched or unregistered BTC metadata still fails closed. Event application and replay additionally require the exact trade symbol, so an order ID reused on another symbol cannot mutate a Bitcoin trade. Existing BTC stop-out policy is unchanged.

An actual recovery installation exposed inherited umask 077 making public extracted inputs unreadable to the non-root Freqtrade process. Extraction now scopes umask 022 to public archive files only; private configuration permissions remain unchanged. Sanitised failure evidence uses the protected instance directory under /var/lib, avoiding Ubuntu's group-writable /var/log root without relaxing parent checks.

The first AWS attempt failed its health gate and the legacy runtime was restored; no database rollback or exchange order was issued by recovery tooling. The restored account was flat and entries remained paused. Recovery wrapper corrections recreate a removed project network and preserve the WAL helper's network_mode=none. These host recovery corrections are not runtime overlays or strategy edits. Final deployment and health evidence must be verified separately against the new merged artifact. Fixes prepared with GPT-6 Astra.
