...
env_file_key_allowed() {
  case "$1" in
    ...
    EXTERNAL_CONFLUENCE_MIN_24H_CHANGE_PCT|EXTERNAL_CONFLUENCE_MIN_PROVIDERS|\
    FEE_PCT_PER_SIDE|FIXED_STOP_PCT|\
    ...
    MONEYFLOW_SPOT_STREAM_STALE_SECONDS|MONEYFLOW_STATUS_PATH|\
    MONEYFLOW_TRADE_LIMIT|OFFHOST_BACKUP_STATUS_PATH|\
    MONITOR_ALLOWED_IPS|\
    ...
    PUBLIC_HTTP_MAX_ATTEMPTS|RELEASE_TAG|REQUIRE_EXTERNAL_CONFLUENCE|REQUIRE_FLOW_CONTEXT|\
    SHARED_HOST_PATH|SIDECAR_HEALTH_PATH|\
    ...
    TELEGRAM_REPORTS_ENABLED|TELEGRAM_PANEL_MODULE|TRADE_SIZE_BASE|TRADE_SIZE_QUOTE|TRAILING_DELTA_BIPS|\
...
REQUIRE_EXTERNAL_CONFLUENCE=${REQUIRE_EXTERNAL_CONFLUENCE:-false}
...
case "$REQUIRE_EXTERNAL_CONFLUENCE" in true|false) ;; *) fail 'REQUIRE_EXTERNAL_CONFLUENCE must be true or false' ;; esac
EXTERNAL_CONFLUENCE_MIN_PROVIDERS=${EXTERNAL_CONFLUENCE_MIN_PROVIDERS:-1}
[[ "$EXTERNAL_CONFLUENCE_MIN_PROVIDERS" == 1 || "$EXTERNAL_CONFLUENCE_MIN_PROVIDERS" == 2 ]] || \
  fail 'EXTERNAL_CONFLUENCE_MIN_PROVIDERS must be 1 or 2'
MONEYFLOW_SPOT_STREAM_STALE_SECONDS=${MONEYFLOW_SPOT_STREAM_STALE_SECONDS:-10}
[[ "$MONEYFLOW_SPOT_STREAM_STALE_SECONDS" =~ ^[0-9]+$ \
   && "$MONEYFLOW_SPOT_STREAM_STALE_SECONDS" -ge 2 \
   && "$MONEYFLOW_SPOT_STREAM_STALE_SECONDS" -le 60 ]] || \
  fail 'MONEYFLOW_SPOT_STREAM_STALE_SECONDS must be from 2 through 60'
...
if [[ "$REQUIRE_EXTERNAL_CONFLUENCE" == true ]]; then
  [[ "$COINGECKO_CONTEXT_ENABLED" == true || "$COINMARKETCAP_CONTEXT_ENABLED" == true ]] || \
    fail 'external confluence requires at least one enabled provider'
  [[ "${REQUIRE_FLOW_CONTEXT:-false}" == true ]] || \
    fail 'external confluence requires REQUIRE_FLOW_CONTEXT=true'
fi
...
[[ ! -L "$ENV_FILE" && $(stat -c '%u' "$ENV_FILE") == 0 \
   && $(stat -c '%g' "$ENV_FILE") == 0 ]] || \
  fail "$ENV_FILE must be a regular root-owned file"
